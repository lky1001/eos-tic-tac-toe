...
const requiredFields = {
  accounts: [{ blockchain: "eos", host: "127.0.0.1", port: 8888 }]
}
...
class Game extends Component {
  ...
  getGameInfo = () => {
    this.eos
      .getTableRows(true, CONTRACT_NAME, this.state.host, "games")
      .then(data => {
        let board = []
        ...
        if (!data.rows || data.rows.length === 0) {
          ...
          this.eos.contract(CONTRACT_NAME, requiredFields).then(contract => {
            contract
              .create(challenger, host, {
                authorization: [challenger, host]
              })
              .then(res => {
                this.setState({
                  isPlay: true,
  ...
  createGame = () => {
    ...
    this.intervalId = setInterval(() => this.getGameInfo(), 1000)
  }
  ...
  restartGame = () => {
    ...
    this.eos.contract(CONTRACT_NAME, requiredFields).then(contract => {
      contract
        .restart(this.state.challenger, this.state.host, this.state.host, {
  ...
  closeGame = () => {
    ...
    this.eos.contract(CONTRACT_NAME, requiredFields).then(contract => {
      contract
        .close(this.state.challenger, this.state.host, {
          authorization: [this.state.challenger, this.state.host, this.state.host]
        })
        .then(res => {
          this.setState({
    ...
    return (
      <div>
        {this.state.isLoading && (
          <div>
            <CircularProgress /> &nbsp;&nbsp;Now Loading...<br />
            <br />
          </div>
        )}
        Tic Tac Toe
        {this.state.isPlay && (
          <div>
            <br />
            Host : {this.state.host}, &nbsp; Challenger : {this.state.challenger} <br />
            <br />
            Turn : {this.state.turn} <br />
            <br />
            <div className="game-board">
              <Board board={this.state.board} handleClick={this.move} />
            </div>
            <Button name="restart" onClick={this.restartGame} variant="contained" color="primary">
              Restart Game
            </Button>{" "}
            <br />
            <br />
            {this.state.turn === this.state.host && (
              <Button name="close" onClick={this.closeGame} variant="contained" color="primary">
                Close Game
              </Button>
            )}
          </div>
        ...
        {!this.state.isLogin && (
          <div>
            <Button name="login" onClick={this.getScatterId} variant="contained" color="primary">
              Login
            </Button>
          </div>
        )}
        {this.state.isLogin &&
          !this.state.isPlay && (
            <div>
              ...
              <Button name="start" onClick={this.createGame} variant="contained" color="primary">
                Create Game
              </Button>
            </div>
          )}
